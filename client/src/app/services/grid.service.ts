    ...
    drawGrid() {
        ...
        for (let x = 0; x < SQUARE_NUMBER; x++) {
            for (let y = 0; y < SQUARE_NUMBER; y++) {
                this.squareColor = 'black';
                // this.squareLineWidth = 0;
                this.fillGridPortion({ x, y }, tiles[y][x].text, tiles[y][x].style.color as string, tiles[y][x].style.font as string);
        ...
        for (let x = 0; x < SQUARE_NUMBER; x++) {
            for (let y = 0; y < SQUARE_NUMBER; y++) {
                if (tiles[x][y].letter !== '') {
                    ...
                    this.squareColor = 'black';
                    // this.squareLineWidth = 0;
                    this.fillGridPortion({ x, y }, tiles[x][y].text, tiles[x][y].style.color as string, tiles[x][y].style.font as string);
                    this.gridContext.strokeRect(x * SQUARE_WIDTH, y * SQUARE_HEIGHT, SQUARE_HEIGHT, SQUARE_WIDTH);
                }
            }
        }
