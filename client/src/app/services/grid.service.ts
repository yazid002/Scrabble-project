...
export class GridService {
    ...
    writeLetter(letter: string, coord: Vec2, throughChat: boolean): void {
        ...
        this.fillGridPortion(
            ...
            tiles[coord.x][coord.y].style.color,
            tiles[coord.x][coord.y].style.font,
        );
        // } else {
    ...
    removeArrow(coord: Vec2) {
        ...
        this.fillGridPortion(coord, tiles[coord.x][coord.y].text, tiles[coord.x][coord.y].style.color, tiles[coord.x][coord.y].style.font);
    ...
    drawGrid() {
        ...
        for (let x = 0; x < SQUARE_NUMBER; x++) {
            for (let y = 0; y < SQUARE_NUMBER; y++) {
                // this.squareColor = 'black';
                // this.squareLineWidth = 0;
                this.fillGridPortion({ x, y }, tiles[y][x].text, tiles[y][x].style.color, tiles[y][x].style.font);
    ...
    drawGridPortionBorder(borderColor: string, coord: Vec2, width: number): void {
        ...
        // );
        this.gridContext.beginPath();
        this.gridContext.moveTo((DEFAULT_WIDTH / SQUARE_NUMBER) * coord.y + 5, (DEFAULT_WIDTH / SQUARE_NUMBER) * coord.x + 5);
        ...
        for (let x = 0; x < SQUARE_NUMBER; x++) {
            for (let y = 0; y < SQUARE_NUMBER; y++) {
                if (tiles[x][y].letter !== '') {
                    ...
                    // this.squareColor = 'black';
                    // this.squareLineWidth = 0;
                    this.fillGridPortion({ x, y }, tiles[x][y].text, tiles[x][y].style.color, tiles[x][y].style.font);
                    this.gridContext.strokeRect(x * SQUARE_WIDTH, y * SQUARE_HEIGHT, SQUARE_HEIGHT, SQUARE_WIDTH);
                }
            }
        }
